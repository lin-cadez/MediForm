...
export default function MultiSelectInput({ predefinedOptions, value, onChange }: MultiSelectInputProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    switch (e.key) {
      case "Enter":
        ...
        } else if (inputValue.trim() && !selectedOptions.includes(inputValue.trim())) {
  ...
  const availableOptions = predefinedOptions.filter(
    (option) => !selectedOptions.includes(option) && option.toLowerCase().includes(inputValue.toLowerCase()),
  )
    ...
    <div className="relative mb-4" ref={dropdownRef}>
      <div className="min-h-[2.5rem] p-2 border border-violet-200 rounded-lg bg-white focus-within:ring-2 focus-within:ring-violet-500/20 focus-within:border-violet-300 transition-all duration-200">
        ...
        <div className="flex items-center gap-2">
          <Input
            ...
            placeholder={selectedOptions.length === 0 ? "Vpišite ali izberite možnosti..." : "Dodajte več..."}
          ...
          >
            <ChevronDown
              className={cn("h-4 w-4 text-slate-500 transition-transform duration-200", isDropdownOpen && "rotate-180")}
            />
          </Button>
        </div>
      </div>
      ...
      {isDropdownOpen && (
        <div className="absolute z-[9999] w-full mt-1 bg-white border border-violet-200 rounded-lg shadow-xl max-h-60 overflow-auto">
          {availableOptions.length > 0 ? (
          ...
          ) : inputValue.trim() && !selectedOptions.includes(inputValue.trim()) ? (
            <button
              type="button"
            ...
            <div className="px-3 py-2 text-sm text-slate-500">Ni razpoložljivih možnosti</div>
          )}
        </div>
      )}
    </div>
